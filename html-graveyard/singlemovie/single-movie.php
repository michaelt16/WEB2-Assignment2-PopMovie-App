...
if (
  ...
  header("Location: movie-error.php");
} else {
  //this is where any relevant writeup can be. Eventually we will likely want to make movie objects, movie presenter objects, etc. to write in the markup.
  header("Location: ../details/view/moviedetailsview.php");
  //REMINDER: KEY IN QUERY STRING IS A STRING, NOT INT
}
...
function idNotFound()
{
  ...
  $id = array_search(intval($_GET["id"]), $allIds, strict: true);
  //var_dump($id);
  return $id === false;
}
...
<body>
<?php require "../../header/moviedetailsheader.php"?>
